...
export default function Cart({ open, openModal, closeModal }) {
  const { cart, showCartItems, setShowCartItems, showCart, checkout } =
    useCart();
  const totalCents = cart.reduce((sum, entry) => {
    return sum + parseFloat(entry?.item?.price) * entry.quantity;
  }, 0);
  ...
  return (
    <section className={classnames({ invisible: !showCart })}>
      ...
      <Cartform open={open} openModal={openModal} closeModal={closeModal} />
    </section>
  );
}
